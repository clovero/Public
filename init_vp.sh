...
runOpenVpn() {
    port=$1
    type=$2
    docker run --name "openvpn:$type$port" --if-not-exists\
...
docker run -d --name v2ray --if-not-exists -v /root/config.json:/etc/v2ray/config.json -p 51888:51888 v2fly/v2fly-core run -c /etc/v2ray/config.json
